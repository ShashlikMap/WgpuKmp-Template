...
final class MetalMapUIView: UIView {
    ...
    private(set) var wgpuAppApi: WgpuAppApi?
    ...
    override func layoutSubviews() {
        ...
        if bounds.width > 0, bounds.height > 0 {
            ...
            wgpuAppApi?.resize(width: UInt32(bounds.width), height: UInt32(bounds.height))
        }
    }

    private func initializeApiIfNeeded() {
        guard wgpuAppApi == nil else { return }
        let viewPtr = Unmanaged.passUnretained(self).toOpaque()
        let layerPtr = Unmanaged.passUnretained(self.layer).toOpaque()

        wgpuAppApi = MainViewControllerKt.createWgpuAppApiForIos(view: UInt64(UInt(bitPattern: viewPtr)), metalLayer: UInt64(UInt(bitPattern: layerPtr)))
